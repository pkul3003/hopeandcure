...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `patient_address_details_view` AS (select `p`.`UHID` AS `UHID`,`p`.`FirstName` AS `FirstName`,`p`.`MiddleName` AS `MiddleName`,`p`.`LastName` AS `LastName`,`p`.`DOB` AS `DOB`,`p`.`Gender` AS `Gender`,`p`.`ContactNumber` AS `ContactNumber`,`p`.`EmailId` AS `EmailId`,`p`.`Aadhar` AS `Aadhar`,`a`.`AddressLine1` AS `AddressLine1`,' ' AS `Name_exp_11`,`a`.`AddressLine2` AS `AddressLine2`,' ' AS `Name_exp_13`,`a`.`Landmark` AS `Landmark`,' ' AS `Name_exp_15`,`a`.`Area` AS `Area`,' ' AS `Name_exp_17`,`a`.`City` AS `City`,' ' AS `Name_exp_19`,`a`.`District` AS `District`,' ' AS `Name_exp_21`,`a`.`State` AS `State`,' ' AS `Name_exp_23`,`a`.`PINCode` AS `PINCode`,' ' AS `Name_exp_25`,`a`.`EmergencyContactNumber` AS `EmergencyContactNumber`,concat(`a`.`AddressLine1`,' ',`a`.`AddressLine2`,' ',`a`.`Landmark`,' ',`a`.`Area`,' ',`a`.`City`,' ',`a`.`District`,' ',`a`.`State`,' ',`a`.`PINCode`,' ',`a`.`EmergencyContactNumber`) AS `concat_address` from (`patients` `p` join `patient_address` `a`) where (`p`.`UHID` = `a`.`UHID`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-12-31 22:32:05
